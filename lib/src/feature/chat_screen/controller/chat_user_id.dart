...
class ChatUserController extends StateNotifier<String?> {
  ...
  FutureEither<String> createConnection({
    required BuildContext context,
    required String phone,
    required String email,
    required String name,
    ...
    try {
      ...
      result.fold(
        ...
        (response) {
          final Map<String, dynamic> responseData = jsonDecode(response.body);
          connectionId = responseData['data']['_id'];
        },
      );

      if (connectionId != null) {
        return Right(connectionId!);
      } else {
        return Left(Failure(message: "Profile update failed"));
      }
    } catch (e, stacktrace) {
      log('Error: $e', name: 'ChatUserControllerError');
      log('Stacktrace: $stacktrace', name: 'ChatUserControllerStacktrace');
      return Left(
        Failure(message: "An Exception Occurred"),
      );
    }
  }
}
