...
import 'package:studio_partner_app/src/feature/studioSchedule/repository/studioAvailability.dart';
import 'package:studio_partner_app/utils/snackbar_service.dart';

final studioAvailabilityControllerProvider =
    StateNotifierProvider<StudioAvailabilityController, StudioAvailability?>(
  (ref) {
    final repo = ref.watch(studioAvailabilityRepoProvider);
...
class StudioAvailabilityController extends StateNotifier<StudioAvailability?> {
  final StudioAvailabilityRepo _repo;
  final Ref _ref;
  StudioAvailabilityController(
      {required StudioAvailabilityRepo repo,
      required Ref ref,
      StudioAvailability? data})
      : _repo = repo,
        _ref = ref,
        super(data);
  ...
  Future<void> getAvailability({
    ...
    try {
      final result = await _repo.getAvailability(studioDocId: studioDocId);
      ...
      result.fold(
        ...
        (response) {
          ...
          if (success) {
            ...
            final availabilityData = StudioAvailability.fromJson(data['data']);
